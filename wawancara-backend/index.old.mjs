...
async function getQuestions() {
  const questions = {}
  const results = await runQueryWithRetry(`SELECT ccat_psy_bank_soal_pertanyaan.*, urut_group id_hrm_competency_dimension_level from ccat_psy_bank_soal_pertanyaan left join ccat_generic_psy_parameter on id_ccat_generic_psy_parameter = id_reference where ccat_psy_bank_soal_pertanyaan.id_ccat_jenis_ujian = 600`)
  for (const result of results) {
    if (!questions[result.id_hrm_competency_dimension_level]) {
      questions[result.id_hrm_competency_dimension_level] = {}
    }
    if (!questions[result.id_hrm_competency_dimension_level][result.param1]) {
      questions[result.id_hrm_competency_dimension_level][result.param1] = []
    }
    questions[result.id_hrm_competency_dimension_level][result.param1].push(result)
  }
  return questions
}
...
app.post('/chat', verifyToken, async (req, res) => {
  const { id_ccat_assesment_batch_jenis_ujian, nip, message, currentHistory, state, param1, interaction } = req.body;
  const soalidx = 0
  ...
  const q = questionC1[state][param1][soalidx]
  let messages = [
...
app.post('/tts', verifyToken, async (req, res) => {
